...
<HTML>
 ...
 function insertRows(){ 
  ...
  var tempRow=0; 
  var tbl=document.getElementById("dictTbl");
  tempRow=tbl.rows.length; //获取当前table 的行数
  //console.info(tempRow);
  var Rows=tbl.rows;//类似数组的Rows 
  var newRow=tbl.insertRow(tbl.rows.length);//插入新的一行 
  var Cells=newRow.cells;//类似数组的Cells 
  //console.info(Cells.length);
  //console.info(newRow.rowIndex);
  for (i=0;i<3;i++)//每行的3列数据 
  { 
  		
     var newCell = Rows[newRow.rowIndex].insertCell(Cells.length); //添加一行
		...
		<table cellSpacing="1" cellPadding="0" width="90%" align="center" bgColor="#f5fafe" border="0">
			<TBODY>
				...
				<tr>
					...
					<!-- struts标签的下拉菜单 -->
					<s:select list="#request.systemDDLList"  name="keyWord"  id="keyWord"
